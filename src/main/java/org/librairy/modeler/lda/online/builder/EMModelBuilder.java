 ...
 */
@Component
public class EMModelBuilder implements ModelBuilder {

    private static final Logger LOG = LoggerFactory.getLogger(EMModelBuilder.class);
    ...
    public LDAModel train(Integer size, Integer vocabSize, Double alpha, Double beta, Integer topics, Integer
        ...
        LOG.info("building a new LDA model by using the EM-LDA optimizer..");
...
//                .option("inferSchema", "true") // Automatically infer data types
        ...
        LOG.info("Dataframe Schema: " + df.schema());
...
//        PipelineModel model = pipeline.fit(df);
        ...
        JavaPairRDD<Long, Vector> documents = cvModel.transform(filteredWords)
                .select("features")
                .toJavaRDD()
                .map(row -> ((Vector) row.get(0)).toDense().compressed())
                .zipWithIndex()
                .mapToPair(r -> new Tuple2<Long,Vector>(Long.valueOf(r._2.intValue()),r._1))
...
//                .setCheckpointInterval(2)
        ...
        // EM Optimizer
        Object out = lda.run(documents);
        LOG.info("Instance of: " + out);
        DistributedLDAModel ldaModel = (DistributedLDAModel) out;
        ...
        for (Tuple2<int[], double[]> topic : topicIndices){
            ...
            for (int i=0; i< topicWords.length;i++){
                ...
                if (wid > vocabArray.length) LOG.warn("\t<missing-"+wid+">\t:"+weights[i]);
                else LOG.info("\t"+vocabArray[wid] +"\t:" + weights[i]);
            }
            LOG.info("------------------------------------------");
        }

        try{
            LOG.info("-- topic matrix --");
            Matrix topicsMatrix = ldaModel.topicsMatrix();
            for (int topic = 0; topic < topics; topic++) {
                LOG.info("Topic " + topic + ":");
                for (int word = 0; word < ldaModel.vocabSize(); word++) {
                    LOG.info(" " + topicsMatrix.apply(word, topic));
                }
                LOG.info("------------------------------------------");
            }
        }catch (Exception e){
            LOG.warn("Catch exception on topic matrix", e);
        }
